...
# Contributing to Meta VR OS SDK Samples

Thanks for your interest in contributing to the Meta VR OS SDK samples!
...
- `JavaSoftwareDevelopmentKitSamples/<name>/` — Meta VR OS Java Software Development Kit (JSDK) samples, typically in Java and/or Kotlin.
- `NativeSoftwareDevelopmentKitSamples/<name>/` — Meta VR OS Native Software Development Kit (NSDK) samples, typically in C and/or C++.
- `UtilityLibrarySamples/<name>/` — Meta VR OS Utility Library samples, typically in Java and/or Kotlin.
- `MixedSdkSamples/<name>/` — Samples that combine two or more Meta VR OS SDK packages.
...
- Build configuration:
  - Native: `CMakeLists.txt` resolving `metavr-os-nsdk` via Prefab
  - JVM: `build.gradle.kts` declaring the `metavr-os-jsdk` dependency
