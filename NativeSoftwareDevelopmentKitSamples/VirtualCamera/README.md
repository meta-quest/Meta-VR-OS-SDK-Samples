...
The Meta VR OS NSDK is distributed as an AAR containing header files and a stub
...
### Step 3: Add the Meta VR OS NSDK Gradle dependency

Add the Meta VR OS NSDK to your version catalog (`gradle/libs.versions.toml`):
...
`ANDROID_STL=c++_static` links the C++ runtime into the binary. The NDK default
is `c++_shared`, which expects `libc++_shared.so` to be packaged alongside the
binary. Because you push this sample to the device as a standalone executable
rather than installing an APK, that library isn’t present and the binary fails
to start with a linker error.
...
project. With Prefab enabled, Gradle exposes the Meta VR OS NSDK to CMake
automatically. The `find_package(metavr-os-nsdk REQUIRED CONFIG)` call in the CMakeLists
resolves without extra configuration. The sample links against
`metavr-os-nsdk::hzos`, which provides both the stub library and header files
(headers are exported transitively).

### Step 5: Update the manifest

Add the `metavr` XML namespace, declare the Meta VR OS SDK version, and add the
...
The [`<metavr:uses-metavr-sdk>`](https://developers.meta.com/horizon/documentation/native/native-manifest-config) element declares which Meta VR OS SDK versions your
app supports. `minSdkVersion` is the lowest version required. `targetSdkVersion`
is the version you designed and tested against. See the [Meta VR OS SDK versioning documentation](https://developers.meta.com/horizon/documentation/android-apps/metavr-os-sdk-versioning/) for more information.
...
First, extract the Meta VR OS NSDK headers and stub library from the AAR (optionally, change its
extension to .ZIP so tools more easily recognize it):

```
metavr-os-nsdk-<version>.aar (extract as ZIP)
└── prefab/
    └── modules/
        ├── hzos_headers/include/metavr/...
...
overview of the API, see the [Virtual Camera Publisher](https://developers.meta.com/horizon/reference/metavr-os-nsdk/latest/virtual_camera_8h) reference documentation.
...
the thread pool yourself before creating the `MvrVirtualCameraManager`. The
...
is needed, but the call to `MvrVirtualCamera_create` returns
`MVR_VIRTUAL_CAMERA_STATUS_PERMISSION_DENIED` if it’s missing.

### Manager and configuration (main)

The `main()` function creates the manager with `MvrVirtualCameraManager_create`, then
defines two input stream configurations (1080p and 720p) using
`MvrVirtualCameraInputConfiguration_init`. You must initialize each struct
...
- `cameraSource`: `MVR_VIRTUAL_CAMERA_SOURCE_APP_DEFINED` (the only source
  available to third-party apps)
- `lensFacing`: `MVR_VIRTUAL_CAMERA_LENS_FACING_EXTERNAL`
...
The `MvrVirtualCamera_create` function registers the camera with the system. This call can
block for up to 5 seconds if the virtual camera service is still starting.
After registration, `MvrVirtualCamera_getCameraId` retrieves the ID that
...
`MvrVirtualCamera_destroy` (which triggers `onStreamClosed` for any active
streams) followed by `MvrVirtualCameraManager_destroy`.
