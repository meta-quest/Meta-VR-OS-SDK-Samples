...
#include <metavr/virtualcamera/VirtualCamera.h>
...
int main() {
  ...
  // --- 1. Create manager ---------------------------------------------------
  MvrVirtualCameraManagerHandle manager = nullptr;
  MvrVirtualCameraStatus status = MvrVirtualCameraManager_create(&manager);
  if (status != MVR_VIRTUAL_CAMERA_STATUS_OK) {
  ...
  // --- 2. Define input stream configurations --------------------------------
  MvrVirtualCameraInputConfiguration cfg1080p;
  MvrVirtualCameraInputConfiguration_init(&cfg1080p, MVR_VIRTUAL_CAMERA_INPUT_CONFIGURATION_LATEST);
  cfg1080p.width = 1920;
  cfg1080p.height = 1080;
  cfg1080p.format = AHARDWAREBUFFER_FORMAT_R8G8B8A8_UNORM;
  cfg1080p.maxFps = 30;

  MvrVirtualCameraInputConfiguration cfg720p;
  MvrVirtualCameraInputConfiguration_init(&cfg720p, MVR_VIRTUAL_CAMERA_INPUT_CONFIGURATION_LATEST);
  cfg720p.width = 1280;
  cfg720p.height = 720;
  cfg720p.format = AHARDWAREBUFFER_FORMAT_R8G8B8A8_UNORM;
  cfg720p.maxFps = 30;

  const MvrVirtualCameraInputConfiguration* inputConfigs[] = {&cfg1080p, &cfg720p};

  // --- 3. Build camera configuration ----------------------------------------
  MvrVirtualCameraConfiguration config;
  MvrVirtualCameraConfiguration_init(&config, MVR_VIRTUAL_CAMERA_CONFIGURATION_LATEST);
  ...
  config.cameraSource = MVR_VIRTUAL_CAMERA_SOURCE_APP_DEFINED;
  config.lensFacing = MVR_VIRTUAL_CAMERA_LENS_FACING_EXTERNAL;
  config.cameraName = "Sample Virtual Camera";
  config.clientData = &app;

  // --- 4. Register virtual camera -------------------------------------------
  MvrVirtualCameraHandle camera = nullptr;
  status = MvrVirtualCamera_create(manager, &config, &camera);
  if (status != MVR_VIRTUAL_CAMERA_STATUS_OK) {
    LOGE("Failed to register virtual camera: %d", status);
    MvrVirtualCameraManager_destroy(manager);
    return 1;
  }

  const char* cameraId = nullptr;
  MvrVirtualCamera_getCameraId(camera, &cameraId);
  if (cameraId) {
  ...
  MvrVirtualCamera_destroy(camera);
  MvrVirtualCameraManager_destroy(manager);
